...
def procesarDig(listadig1,listadig2):
    ...
    for i in range(len(listadig1)):
        for k in range(len(listadig2)):
            #analizo las dos listas recorriendo la listadig2 primero
            if listadig2[k] == listadig1[i] and i == k:
                #si tienen el mismo indice significa que están en el mismo lugar y el digito es correcto, haciendo la variable encontrado True
                cont += 1
                encontrado = True
            #proceso ambas listas y si encuentro el valor,y a la vez no estan en la misma posición sumo uno al contador de aproximados, pido un encontrado Falso para que no se procese el correcto
            if listadig1[i] == listadig2[k] and i != k and encontrado == False :
                aprox += 1
            encontrado = False
            #Parche para el programa en caso de que se repita un número
            while cont + aprox > 4:
                aprox -= 1
    return cont, aprox
